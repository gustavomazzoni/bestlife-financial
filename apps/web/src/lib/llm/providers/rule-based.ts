...
// Recurrence keyword -> frequency mapping (checked in order, most specific first)
const RECURRENCE_PATTERNS: Array<{
  pattern: RegExp;
  frequency: ScheduleFrequency;
}> = [
  {
    pattern: /\b(todo\s+m[eê]s|mensal(mente)?)\b/,
    frequency: ScheduleFrequency.MONTHLY,
  },
  {
    pattern: /\b(toda\s+semana|semanal(mente)?)\b/,
    frequency: ScheduleFrequency.WEEKLY,
  },
  {
    pattern: /\b(todo\s+ano|anual(mente)?)\b/,
    frequency: ScheduleFrequency.YEARLY,
  },
  // Generic "recurring/fixed" phrasing with no explicit cadence defaults to monthly (bills are the common case)
  {
    pattern: /\b(recorrente|fixo|fixa)\b/,
    frequency: ScheduleFrequency.MONTHLY,
  },
];
...
function parseAmount(normalizedText: string): number | null {
  ...
  // "dia 28" is a due-date day, not an amount — exclude it so a phrase
  // like "Pagar 120 de luz, vence dia 28 todo mês" doesn't pick up 28.
  const amountMatches = matches.filter(
    m => !/\bdia\s*$/.test(normalizedText.slice(0, m.index).trimEnd())
  );
  const candidates = amountMatches.length > 0 ? amountMatches : matches;

  if (candidates.length > 0) {
    // Take the last number found (usually the amount is at the end)
    const lastMatch = candidates[candidates.length - 1][1];
...
/**
 * Match category based on keywords against the categories passed in
 * (fetched from the DB by the caller — this provider has no DB access).
 */
...
/**
 * Match an account by name against the text (e.g. "no Itaú", "na carteira").
 */
function matchAccount(lowerText: string, accounts: DBAccount[]): string | null {
...
/**
 * Detect recurrence phrasing ("todo mês", "recorrente", "toda semana", ...).
 */
function detectRecurrence(lowerText: string): {
  isRecurring: boolean;
...
function parseDate(lowerText: string): Date {
  ...
  // "dia N" (day of month, e.g. "vence dia 28") — assume current month,
  // rolling to next month if that day has already passed this month.
  const dayOfMonthPattern = /\bdia\s+(\d{1,2})\b/;
  match = lowerText.match(dayOfMonthPattern);
  if (match) {
    const day = parseInt(match[1], 10);
    if (day >= 1 && day <= 31) {
...
function generateDescription(text: string): string {
  ...
  description = description.replace(
    /\b(hoje|ontem|anteontem|amanhã|amanha|today|yesterday|tomorrow)\b/gi,
    ''
  );
...
export class RuleBasedProvider implements LLMInferenceProvider {
  async inferTransaction(
    ...
    const category = matchCategory(normalizedText, type, categories);
    ...
    const accountId = matchAccount(normalizedText, accounts);
    const { isRecurring, frequency } = detectRecurrence(normalizedText);
