...
const scheduledFormSchema = z
  .object({
    ...
    frequency: z.enum(['ONCE', 'WEEKLY', 'MONTHLY', 'YEARLY'] as const, {
...
interface ScheduledFormProps {
  mode: 'create' | 'edit';
  initialData?: Partial<ScheduledFormData>;
  onSubmit: (data: ScheduledFormData) => Promise<void>;
  onCancel: () => void;
}
...
const frequencyOptions: { value: ScheduleFrequency; label: string }[] = [
  { value: 'ONCE', label: 'Uma vez (agendamento único)' },
...
export function ScheduledForm({
  ...
  const isRecurring = currentFrequency && currentFrequency !== 'ONCE';
  ...
  const onFormSubmit = async (data: ScheduledFormData) => {
    try {
      await onSubmit(data);
    } catch (err) {
      setError('root', {
      ...
      <form onSubmit={handleSubmit(onFormSubmit)} className="space-y-5">
        ...
        >
          <Input
            ...
            placeholder="Ex: Aluguel, Salário, Consulta médica"
          />
        </Field>
        ...
        {/* Date — label changes based on frequency */}
        <Field
          label={isRecurring ? 'Data de início' : 'Data agendada'}
          id="startDate"
